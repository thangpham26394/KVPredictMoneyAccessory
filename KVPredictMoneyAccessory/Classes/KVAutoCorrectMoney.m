...
@implementation KVAutoCorrectMoney
...
+ (NSArray *)potentialMoneyWith:(double)money shouldDisplayDefault:(BOOL)shouldDisplayDefault {
    ...
    BOOL shouldAdd500 = money > 500000;
    if (shouldAdd500) {
        m -= 500000;
    }
    double max = 1000000;
    ...
    NSArray *VNMoneyType = @[@(10000),@(20000),@(50000),@(100000),@(200000),@(500000)];
        ...
        if (![potentialArray containsObject:@(addValue)]) {
            ...
            if ([VNMoneyType containsObject:@(addValue)]) {
                ...
                for ( NSUInteger j = 0;  j < VNMoneyType.count; j++) {
                     ...
                     */
                    if (start != tienchan && loaiTien == 50000) {
                        shouldNotAdd = YES;
                    }
                }
                if (!shouldNotAdd) {
    ...
    if (shouldDisplayDefault) {
        ...
        //Remove giá trị tiền mặc định nếu không set chế độ hiển thị giá tiền mặc đinh không kể chẵn lẻ
        if ([potentialArray containsObject:@(m)]) {
            [potentialArray removeObject:@(m)];
        }
    }
    ...
    if (shouldAdd500) {
        for (int i=0; i<potentialArray.count; i ++) {
            double newValue = [potentialArray[i] doubleValue] + 500000;
